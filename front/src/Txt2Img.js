...
export default function Txt2Img() {
  ...
  useEffect(() => {
    ...
    let timer = setInterval(() => {
      ...
      fetch('https://dev-common-s3.s3.ap-northeast-1.amazonaws.com/arts/' + params.fileName + '-1.png')
        .then((res) => {
          if (res.status == 200) {
            console.log('DEAD BEEF GET!');
            //setImgUrl('https://sd.tokyo-tsushin.com/img/' + params.fileName);
            setImgUrl('https://dev-common-s3.s3.ap-northeast-1.amazonaws.com/arts/' + params.fileName + '-1.png');
            setLoading(false);
            clearInterval(timer);
	  } else {
